...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=100, nAux=5, zero_init_residual=False):
        ...
        auxiliary_classifiers_list = [
            MainClassifier(channel=512 * block.expansion,
                           num_classes=num_classes),
        ]
        auxiliary_classifiers_list += [AuxiliaryClassifier(
            channel=512 * block.expansion, num_classes=num_classes) for _ in range(nAux)]
    ...
    def forward(self, inputs):
        if self.training:
            ...
            for index in range(len(inputs)):
                ...
                out_list.append(
                    self.auxiliary_classifiers_list[index](layer4_out))
                if index == 0:
                    # return layer4_out
                    self.record_bn()
            self.load_bn()
            ...
            layer3_out = self.layer3(layer2_out)
            layer4_out = self.layer4(layer3_out)
            return self.auxiliary_classifiers_list[0](layer4_out)


def resnet18(pretrained=False, nAux=5, **kwargs):
    """Constructs a ResNet-18 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], nAux=nAux, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(pretrained=False, nAux=5, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3], nAux=nAux, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet34']))
    return model


def resnet50(pretrained=False, nAux=5, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], nAux=nAux, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    return model


def resnet101(pretrained=False, nAux=5, **kwargs):
    """Constructs a ResNet-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], nAux=nAux, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet101']))
    return model


def resnet152(pretrained=False, nAux=5, **kwargs):
    """Constructs a ResNet-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 8, 36, 3], nAux=nAux, **kwargs)
    if pretrained:
...
def reverseAux(nAux=5):
    """Construct reverseAux model which use Aux conv right after input.

    Args:
        nAux (int, optional): Number of Auxiliary conv. Defaults to 5.
    """
    model = reverseAuxModel(nAux=nAux)
    return model


class reverseAuxModel(nn.Module):
    def __init__(self, nAux=5, nOut=100):
        super(reverseAuxModel, self).__init__()
        self.AuxExtractor = nn.ModuleList(
            [self.extractor().cuda() for _ in range(nAux)])
    ...
    def forward(self, x):
        x = self.AuxExtractor[0](x)
        x = self.linear(x)
        return x

    def auxForward(self, AuxData):
        assert self.training and "This function should only be used in training process!"
        features = []
        for data, aux in zip(AuxData, self.AuxExtractor):
            features.append(aux(data))
        average = torch.stack(features).mean(dim=0)
        x = self.linear(average)
        return x, *features
