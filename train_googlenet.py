...
from utilities.lossFunction import dynamicWeightedCELoss, softlabelCELoss
...
if args.noise >= 0:
    ...
    transforms.Resize(image_shape), transforms.ToTensor(),transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    ...
    if args.dynamicWeightLoss or args.mixLabel:
        transform_trainlabel.append(highPassNoiseTarget(noiseSet,srng=SRNG(0xC8763,len(noiseSet))))
elif args.dynamicWeightLoss or args.mixLable:
        print("The custom loss function or mix label can only be used with high noise transform")
transform_test = [
    transforms.Resize(image_shape),
    transforms.ToTensor(),
...
# net = models.googlenet( pretrained = True,aux_logits = True).to(device)# Notice the aux logits will be turned off in eval mode
net = models.inception_v3( pretrained = True,aux_logits = True).to(device)# Notice the aux logits will be turned off in eval mode
if device == 'cuda':
...
optimizer = torch.optim.SGD(net.parameters(), lr=args.lr, momentum=0, dampening=0, weight_decay=0, nesterov=False)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        outputs, aux = net(inputs) #x, aux2, aux1
        if batch_idx % 10 == 0:
            loss = aux_criterion(aux, targets, [1 - args.eps, args.eps])
            # loss = criterion(aux, targets[:,1])
        else:
            loss = criterion(outputs, targets[:,0])
        # loss = criterion(outputs, targets[:,0]) + criterion(aux, targets[:,1])
        loss.backward()
        optimizer.step()
